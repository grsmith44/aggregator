...
func (cfg *apiConfig) fetchAndProcessFeeds(ctx context.Context, batchSize int) {
	...
	for _, feed := range feedList {
		...
		go func(feed Feed) {
			defer wg.Done()
			cfg.processFeed(ctx, feed)
			cfg.DB.MarkFeedAsFetched(ctx, feed.ID)
		}(feed)
	}
}

func (cfg *apiConfig) processFeed(ctx context.Context, feed Feed) {
	client := &http.Client{
	...
	for _, item := range rss.Channel.Items {
		cfg.addPostToDatabase(ctx, item, feed)
	}
}
